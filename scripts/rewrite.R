...
# Assignment 2:  
MTweedieTests <- function(N, M, sig, num_cores = 8) {
  # Split the simulations into multiple cores
  results <- mclapply(1:num_cores, function(core) {
    # Calculate simulations per core
    simulations_per_core <- M / num_cores
    share_reject_core <- replicate(simulations_per_core, simTweedieTest(N))
    sum(share_reject_core < sig) / simulations_per_core
  })
  
  # Combine results from all cores
  share_reject <- mean(unlist(results))
